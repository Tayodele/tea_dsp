...
def  main():
  ...
  installer = "\installer\TeaFilter Installer.exe"
   
  if demo:
    installer = "\installer\TeaFilter Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\TeaFilter manual.pdf" 
  ]
  ...
  ZIPNAME = "TeaFilter-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "TeaFilter-v" + FULLVERSIONSTR + "-win-demo.zip"
