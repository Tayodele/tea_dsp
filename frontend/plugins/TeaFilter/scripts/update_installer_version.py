...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=TeaFilter Demo Installer\n"
      else:
        line="OutputBaseFilename=TeaFilter Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the TeaFilter Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the TeaFilter installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=TeaFilter Demo installer\n"
     else:
       line="SetupWindowTitle=TeaFilter installer\n"
